Development of qgis-planscape-plugin plugin
===========================

This project uses [qgis_plugin_tools](https://github.com/OurPlanscape/qgis_plugin_tools) submodule,
so set git setting value: `git config --global submodule.recurse true`.

When cloning use `--recurse-submodules` like so:
`git clone --recurse-submodules https://github.com/OurPlanscape/qgis-planscape-plugin.git`
...
The code for the plugin is in the [planscape](../qgis-planscape-plugin) folder. Make sure you have required tools, such as
Qt with Qt Editor and Qt Linquist installed by following this
[tutorial](https://www.qgistutorials.com/en/docs/3/building_a_python_plugin.html#get-the-tools).

For building the plugin use platform independent [build.py](../qgis-planscape-plugin/build.py) script.
...
* they contain absolute imports:
    ```python
    from planscape.utils.exceptions import TestException # Good

    from ..utils.exceptions import TestException # Bad

    ```
* they will be found by [build.py](../planscape/build.py) script (`py_files` and `ui_files` values)

* you consider adding test files for the new functionality
## Deployment

Edit [build.py](../planscape/build.py) to contain working values for *profile*, *lrelease* and *pyrcc*. If you are
...
* Add this badge ![](https://github.com/OurPlanscape/qgis-planscape-plugin/workflows/Translations/badge.svg) to
...
The translation files are in [i18n](../qgis-planscape-plugin/resources/i18n) folder. Translatable content in python files is
